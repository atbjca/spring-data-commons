.PHONY: clean build test install deploy help

# =============================================================================
# Spring Data Commons 2.7.x BJCA/NES 维护分支 — 构建快捷命令（Maven）
# =============================================================================
# 本项目为 Maven 工程，编译产物字节码目标为 Java 8。本地可用 JDK 17（Corretto）。
# 私服依赖解析与发布配置（mirror、bjca profile 属性、snapshots 凭证）全部由
# Maven 默认用户级配置 ~/.m2/settings.xml 提供，且 bjca profile 默认激活，
# 因此不显式传 -s：项目内 settings.xml 是 Spring 官方原版（仅含 Spring Artifactory
# 认证），对内网 Nexus 场景无用。如需覆盖，命令行传 SETTINGS='-s <路径>' 即可。
