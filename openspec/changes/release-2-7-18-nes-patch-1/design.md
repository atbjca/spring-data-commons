...
`spring-data-commons-2.7` is a Git repository at `spring-data-commons-2.7`. This change releases `2.7.18-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.
...
- Reuse qualifying development build/test evidence only after auditing commit ancestry and the release-only diff, then verify publication metadata and consumer behavior locally.
...
Only the manifest owner for `spring-data-commons-2.7` may write to this repository while the component is being prepared.
...
### 3. Bind proportional validation and publication to one release commit

The approved exception for this release reuses these committed development gates:

- the security remediation full suite: 3328 tests, 0 failures, 0 errors;
- the later full regression after switching to the NES Framework dependency: 0 failures and 0 errors.

`make build` and `make test` are not repeated only if ancestry and the full release diff prove there are no production or test source changes after that evidence. Any such source change invalidates the exception and requires the affected gate to run again.

The coordinating main session must still run a local install serially before deploy. For this release, the approved fast path is the recorded no-clean `./mvnw -DskipTests install` invocation; it still compiles main and test sources while skipping test execution. Scan every generated POM/BOM and run representative RELEASE-only consumer validation after local publication. Update `README.adoc`, `doc/QUICK_START.md`, `doc/USER_MANUAL.md`, `doc/GAV_MAPPING.md`, `doc/REQUIREMENTS.md`, and `doc/VULNERABILITY_REPORT.md` before creating the dedicated release commit. Install and deploy from the same recorded inputs without subsequent source changes.
...
- **Stale development verification is reused incorrectly** — prove the evidence commit is an ancestor and audit the complete intervening and release-only diffs; invalidate the exception on any production or test source change.
