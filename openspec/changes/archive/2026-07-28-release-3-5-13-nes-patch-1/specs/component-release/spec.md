...
The release process MUST assign one active owner to `spring-data-commons-3.5` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-data-commons-3.5`.
...
The component MUST either run its approved build and relevant tests or record an operator-approved reuse exception backed by a full release-only diff review showing no production or test source change. Regardless of that exception, the component MUST freshly complete local publication or effective-POM generation, POM scanning, and representative consumer validation against the exact inputs that become the release commit.

#### Scenario: All local gates pass
- **WHEN** build/test results pass or an approved source-free release-diff exception is recorded, and local installation, the metadata scan, and consumer validation succeed
- **THEN** commands and evidence are recorded and the component may become `locally-verified`

#### Scenario: Reuse exception is invalidated
- **WHEN** the release diff contains a production source, test source, or unrelated build-logic change
- **THEN** build/test reuse is forbidden and the coordinator must determine the required validation before release continues
