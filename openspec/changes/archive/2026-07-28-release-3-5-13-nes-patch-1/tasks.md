...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-data-commons-3.5` in `spring-data-commons-3.5`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.data:bjca-footstone-bpring-data-commons:3.5.13-nes.patch.1`.
...
- [x] 3.1 Record the operator-approved exception not to rerun `make build`; verify the release diff contains no production source or unrelated build-logic change.
- [x] 3.2 Record the operator-approved exception not to rerun `make test`; verify the release diff contains no test source change.
- [x] 3.3 Generate local publications/effective POMs incrementally without `clean` or tests under the coordinating session's global build lock.
...
- [x] 4.3 Create the dedicated release commit for `spring-data-commons-3.5` `3.5.13-nes.patch.1`.
