...
parent（`org.springframework.data.build:spring-data-parent`）MUST 采用方案 A（保留原坐标），版本从 `3.5.14-SNAPSHOT` 锁定为已发布正式版 `3.5.13`，以保证可从私服/本地仓库解析。
...
### Requirement: 上游 spring-framework 依赖替换

本能力 MUST 满足以下要求：上游 spring-framework 依赖替换。

本项目的上游 spring-framework 依赖必须从官方坐标替换为去特征化 NES fork（`cn.bjca.footstone.bpring:bjca-footstone-bpring-*`），并通过导入 fork BOM 统一版本，使制品依赖链不残留官方 GAV 特征。

#### Scenario: 导入 fork BOM
- **WHEN** 检查 `pom.xml` 的 `<dependencyManagement>`
- **THEN** 存在 import 型依赖 `cn.bjca.footstone.bpring:bjca-footstone-bpring-framework-bom`，版本 `6.2.19-nes.patch.1-SNAPSHOT`，`type=pom`、`scope=import`

#### Scenario: spring 依赖坐标替换
- **WHEN** 检查 `pom.xml` 的 spring-framework 依赖
- **THEN** `spring-core/beans/context/expression/tx/oxm/web/webflux/webmvc/core-test` 的 groupId 为 `cn.bjca.footstone.bpring`、artifactId 为 `bjca-footstone-bpring-*`
- **AND** 版本由 BOM 统一管理，依赖声明处不显式写 `<version>`

#### Scenario: 保留无 fork 的依赖
- **WHEN** 检查 `spring-hateoas` 依赖
- **THEN** 保留官方坐标 `org.springframework.hateoas:spring-hateoas`（无对应 NES fork）

#### Scenario: 替换后全量测试通过
- **WHEN** 执行 `./mvnw clean test`
- **THEN** 全量测试通过（3630 tests，Failures 0 / Errors 0），无因依赖替换引入的回归
