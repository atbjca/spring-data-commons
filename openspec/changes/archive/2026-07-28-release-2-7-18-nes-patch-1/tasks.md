...
- [x] 1.1 Confirm this change is apply-ready and assigned exclusively to `spring-data-commons-2.7` in `spring-data-commons-2.7`.
...
- [x] 2.3 Discover the complete local publication set and reconcile it with the representative GAVs: `cn.bjca.footstone.bpring.data:bjca-footstone-bpring-data-commons:2.7.18-nes.patch.1`.
...
- [x] 2.5 Update `README.adoc`, `doc/QUICK_START.md`, `doc/USER_MANUAL.md`, `doc/GAV_MAPPING.md`, `doc/REQUIREMENTS.md`, and `doc/VULNERABILITY_REPORT.md` with version, GAVs, required RELEASE dependencies, validation commands, Nexus location, exclusions, and notable constraints.
- [x] 2.6 Review the full working diff and confirm it contains no unrelated source changes or credentials.

## 3. Local verification

- [x] 3.1 Audit and reuse the committed build/package evidence under the approved release-only-diff exception; rerun only if production or test sources changed.
- [x] 3.2 Audit and reuse the committed full-suite test evidence (3328 tests, 0 failures, 0 errors, plus the later NES Framework regression) under the same exception.
- [x] 3.3 Have the coordinating main session serially generate local publications/effective POMs with the approved no-clean `./mvnw -DskipTests install` command.
...
- [x] 4.3 Create the dedicated release commit for `spring-data-commons-2.7` `2.7.18-nes.patch.1`.
