...
The release process MUST assign one active owner to `spring-data-commons-2.7` and MUST inspect branch, origin, tracked and untracked changes, and active OpenSpec changes before modifying `spring-data-commons-2.7`.
...
The component MUST audit existing development build/test evidence, its commit ancestry, and every intervening change. It MAY reuse qualifying build/test evidence under an explicitly recorded exception only when the release-only diff contains no production or test source changes. It MUST still complete local publication or effective-POM generation, POM scan, and representative consumer validation against the release inputs.

#### Scenario: Approved build and test exception applies
- **WHEN** committed full-suite evidence covers the current production/test sources and the release-only diff changes only version, internal RELEASE dependencies, documentation, OpenSpec, or evidence
- **THEN** `make build` and `make test` may be skipped with the evidence and diff audit recorded

#### Scenario: All required local gates pass
- **WHEN** the approved build/test evidence or required reruns pass, the coordinating main session runs the recorded local install serially, and the metadata scan and consumer validation succeed
- **THEN** commands and evidence are recorded and the component may become `locally-verified`

#### Scenario: A local gate fails
- **WHEN** a required command fails, source changes invalidate reused evidence, or a gate is skipped without an approved exception
- **THEN** the component cannot be committed for release or deployed

### Requirement: Auditable release commit and documentation

The component SHALL have one dedicated release commit containing `2.7.18-nes.patch.1`, internal RELEASE dependency updates, `README.adoc`, `doc/QUICK_START.md`, `doc/USER_MANUAL.md`, `doc/GAV_MAPPING.md`, `doc/REQUIREMENTS.md`, `doc/VULNERABILITY_REPORT.md`, approved exclusions, and no unrelated source changes.
