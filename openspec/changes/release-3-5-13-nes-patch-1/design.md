...
`spring-data-commons-3.5` is a Git repository at `spring-data-commons-3.5`. This change releases `3.5.13-nes.patch.1` from one dedicated source commit and records enough evidence for a fresh session to reconcile the component against Git, OpenSpec, and Nexus.
...
- Verify that the release-only diff qualifies for the operator-approved build/test reuse exception, then freshly verify publication metadata and consumer behavior.
...
Only the manifest owner for `spring-data-commons-3.5` may write to this repository while the component is being prepared.
...
The operator explicitly approved not rerunning `make build` or `make test` for
this RELEASE. The exception is valid only while the full release diff contains
no production source, test source, or unrelated build-logic change and the
existing development validation remains documented. Any such executable change
invalidates the exception and requires coordinator review before continuing.

The exception does not apply to version-sensitive publication validation. The
coordinating main session must serially run an incremental local Maven install
without `clean`, enumerate the actual publication set, scan every generated POM
for internal SNAPSHOT references, and run representative RELEASE-only consumer
validation. Update `README.adoc`, `doc/QUICK_START.md`, `doc/USER_MANUAL.md`,
`doc/GAV_MAPPING.md` before creating the dedicated release commit. Deploy from
that recorded commit without subsequent source changes.
...
- Before Nexus publication, fix release preparation through this change and repeat the required proportional local verification.
