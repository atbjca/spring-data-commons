...
`spring-data-commons-3.5` currently targets `3.5.13-nes.patch.1` but does not yet have a complete, immutable, and independently auditable RELEASE lifecycle. The release must survive session loss, preserve unrelated work, and publish metadata that references only approved internal RELEASE dependencies.

## What Changes

- Freeze and inventory the `spring-data-commons-3.5` worktree before release edits.
...
- Reuse the approved development build/test evidence only after a release-only diff review, then run a fresh local installation, generated-POM scan, and consumer verification.
...
- `component-release-spring-data-commons-3.5`: Release `spring-data-commons-3.5` `3.5.13-nes.patch.1` with reproducible local gates, immutable Nexus publication, exact Git tagging, documentation, and archive evidence.
...
- **Repository:** `spring-data-commons-3.5`
- **Release version:** `3.5.13-nes.patch.1`
- **Representative GAVs:** `cn.bjca.footstone.bpring.data:bjca-footstone-bpring-data-commons:3.5.13-nes.patch.1`
