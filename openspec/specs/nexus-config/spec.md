...
- **WHEN** 执行发布
- **THEN** `${nexusReleaseUrl}` / `${nexusSnapshotUrl}` 由 `~/.m2/settings.xml` 的 `bjca` profile 属性解析
...
#### Scenario: 快照发布
- **WHEN** 执行 `./mvnw -DskipTests clean deploy`
- **THEN** `3.5.13-nes.patch.1-SNAPSHOT` 制品发布到 snapshots 仓库
