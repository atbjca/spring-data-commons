...
The BJCA-maintained NES release is available from the internal Nexus RELEASE
repository:
...
<repository>
  <id>releases</id>
  <url>http://192.168.131.36:8088/repository/releases</url>
</repository>
----
...
This artifact requires the published Spring Framework NES release
`cn.bjca.footstone.bpring:bjca-footstone-bpring-framework-bom:5.3.39-nes.patch.1`.
Its Java packages and JPMS automatic module name remain `org.springframework.data.*`
and `spring.data.commons`; the bytecode target remains Java 8. There are no
release exclusions.

=== Release validation

The release reuses the committed development validation of 3328 passing tests
and the later full regression against the NES Framework dependency. The
release-only diff must contain no production or test source changes. Before a
single coordinator-controlled deploy, run the local publication gate:
...
Then verify every generated POM has no internal `cn.bjca.footstone`
`-SNAPSHOT` dependency and run a RELEASE-only consumer check. `make deploy`
is reserved for the coordinating release session after Nexus absence is
confirmed. See link:doc/QUICK_START.md[Quick Start] and
link:doc/GAV_MAPPING.md[GAV Mapping] for the complete release coordinates.
